...
<svelte:head>
	<title>Creator Studio - OleksandrMedia</title>
</svelte:head>
	...
	<div class="studio-layout">
		...
		<main class="studio-main">
			<div class="studio-header">
				...
				<p>
					{lang === "ru"
						? `Добро пожаловать, ${user?.username || "Создатель"}!`
			...
			{:else if stats}
					...
					<div class="stat-card">
						...
						<div class="stat-content">
							...
							<span class="stat-value"
								>{stats.totalViews?.toLocaleString() || 0}</span
							>
						</div>
					</div>
				...
				<div class="recent-section">
					<div class="section-header">
						...
						<a href="/studio/videos" class="btn-secondary"
							>{lang === "ru" ? "Показать все" : "View All"}</a
						>
					</div>
								...
								<tbody>
									{#each recentVideos as video}
										<tr>
											...
											<td>{video.likes || 0}</td>
											<td>
												{#await fetch(`/api/comments?videoId=${video.id}`)
													.then((r) => r.json())
													.then((d) => d.comments?.length || 0)}
													...
												{:then count}
													{count}
												{:catch}
