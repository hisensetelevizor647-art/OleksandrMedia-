	...
	async function loadChannel() {
		...
		if (user && channel) {
			const subRes = await fetch(
				`/api/subscriptions?channelId=${channelId}`,
			);
			if (subRes.ok) {
...
<svelte:head>
	<title>{channel?.username || "Channel"} - OleksandrMedia</title>
</svelte:head>
	...
	<main class:sidebar-open={sidebarOpen}>
		...
		{:else if channel}
			<div class="channel-header">
				<div
					class="banner"
					style={channel.banner
						? `background-image: url(${channel.banner})`
						: ""}
				></div>
				<div class="channel-info">
					...
					<div class="info">
						...
						<p class="stats">
							...
							{t.subscribers_other} • {videos.length}
							{t.videos_other}
						</p>
